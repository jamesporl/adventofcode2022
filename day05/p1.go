			...
			for k := 1; k <= moveCount; k += 1 {
				...
				for l := 0; l < len(crates[moveFrom - 1]); l += 1 {
					if (crates[moveFrom - 1][l] == " ") {
						...
						lastCrateFromLetter = crates[moveFrom - 1][l]
						lastCrateFromIdx = l
					}
				}
				crates[moveFrom - 1][lastCrateFromIdx] = " "
				...
				for m := 0; m < len(crates[moveTo - 1]); m += 1 {
					if (crates[moveTo - 1][m] == " ") {
						crates[moveTo - 1][m] = lastCrateFromLetter
						shouldAppend = false
						break
					}
				}
				...
				if (shouldAppend) {
					crates[moveTo - 1] = append(crates[moveTo - 1], lastCrateFromLetter)
				}
			}
		}
	}
